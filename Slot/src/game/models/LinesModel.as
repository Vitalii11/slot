	...
	public class LinesModel extends BaseModel
	{
		...
		private function showBordersInLine():void
		{
			...
			for (var j:int = 0; j < countBorders; j++) 
			{
				...
				borderModel.addToStage(Hlp.gameEnter.topLayer);
				...
				for (var i:int = 0; i < DataStorage.instance().countReels; i++) 
				{
					...
					{
						if (ind == counter) {
							Hlp.gameEnter.gameModel.reelsModel.getReel(i).getSlotAnim(k).startAnimation();
							stop  = true;
						}
						if (stop)
							break;
		...
		private function showLines(e:MouseEvent):void 
		{
			...
			Hlp.gameEnter.ui.updateTxt('Line', String(countLines));
